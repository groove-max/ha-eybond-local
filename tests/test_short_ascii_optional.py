...
from custom_components.eybond_local.payload.short_ascii import ShortAsciiError, parse_f, parse_rb


def _rb(*, voltage=520, soc=80):
    body = bytearray(37)
    for offset, value in ((0, voltage), (3, 30), (5, 0), (7, 250), (9, 13),
...
class OptionalPayloadTests(unittest.TestCase):
    ...
    def test_rb_documented_fields_without_guessing_current_scale_or_battery_power(self):
        values = parse_rb(_rb())
        ...
        for key in ("battery_voltage", "battery_current", "battery_power", "load_power", "bms_charging_current"):
...
class OptionalReadTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_one_optional_query_per_cycle_fair_scheduling_and_full_snapshot(self):
        ...
        self.assertEqual(self.transport.requests,
                         [b"Q1\x01\r", b"RB\x01\r", b"Q1\x01\r", b"F\x01\r", b"Q1\x01\r"])
        self.assertGreater(third.diagnostics["short_ascii_rb_age_seconds"], 1.9)
        self.assertLess(third.diagnostics["short_ascii_rb_age_seconds"], 2.1)
    ...
    async def test_missing_rb_does_not_block_f_and_repeated_failures_use_existing_recheck(self):
        ...
        self.assertEqual((await self.read(125)).values["battery_soc"], 80)
    ...
    async def test_persisted_unsupported_group_is_not_queried_or_projected(self):
        seed_unsupported_commands(self.state, ("short_ascii:RB", "short_ascii:F"))
        result = await self.read(0)
        self.assertEqual(self.transport.requests, [b"Q1\x01\r"])
        self.assertNotIn("battery_soc", result.values)
        self.assertEqual(result.diagnostics["short_ascii_optional_status"], "RB=unsupported; F=unsupported")
