...
requests. Archive creation does not solicit the optional auxiliary PV/MPPT
(`0200`) read, force-enable opt-in PV entities, or enable inverter controls —
those remain a separate runtime path. This is a new diagnostic snapshot, not a
continuous recording: if a reading jumps, also report when it happened and
which sensor was affected. A later archive may no longer contain the response
that caused that jump.
