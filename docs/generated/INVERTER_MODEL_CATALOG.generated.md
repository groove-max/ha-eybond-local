...
- Model catalog version: 2026.09.1
- Runtime inverter catalog version: 2026.08.8
- Models: 18 | Variants: 20

## How To Read This Catalog

This page describes what has been confirmed for a specific inverter model or model family. It is more precise than a simple compatibility list.

- **Supported Models** have a safe built-in runtime path and confirmed telemetry.
- **Limited Or Experimental Models** can still work, but a listed limitation, partial telemetry, unconfirmed controls, or a read-only path remains.
- **Research Queue** records known hardware that does not yet have a safe built-in runtime path.
- **Telemetry** describes confidence in local readings; **Controls** describes confidence in local writes; **Hardware** describes the evidence available from a real device.
- **Protocol**, **Detection**, **Runtime Tier**, descriptors, surfaces, and fingerprints are implementation details useful when comparing variants.
- **Family-level runtime coverage** means EyeBond Local has a safe generic protocol path, but has not claimed an exact commercial model identity.
...
| EyeBond | Short-ASCII family | eybond_short_ascii | anchors | partial | partial | none | captured |
...
| `modbus_smg.protocol_6_family_fallback` | modbus_smg | smg_protocol_6_family_fallback | full | no |
...
### EyeBond — Short-ASCII family (`eybond_short_ascii_family`)

- Lifecycle: experimental
- Aliases: EyeBond Short-ASCII family
...
- Known limitations:
  ...
  - Diagnostic counters: bms_link_loss_count and rb_hard_reject_count are runtime-scoped; reading_hold_pending_count stays 0 (physics confirmation-hold deferred).
  - BMS current / battery_power path is SmartValue-correlated on one live family member; other members (e.g. Maxinn) are not separately live-proven and only light up if RH=1.
- Evidence: 2 source(s)

### Gootu — GT-H2436M14P5 (`gootu_gt_h2436m14p5`)

- Lifecycle: experimental
- Aliases: Gootu Hybrid 24V 3.6kW
- Validation: hardware captured, telemetry confirmed, controls partial
- Coverage: runtime available, cloud correlated, vendor map documented
  - Coverage notes:
    - Runtime identification uses the exact local G-ASCII F/SVFW fingerprint, not the commercial model name.
    - Captured GPDAT0 and GPV responses match the existing EyeBond G-ASCII telemetry decoder.
    - Charging-priority CPR03 and CPR00 writes received ACK responses in a SmartValue proxy capture.
    - The family protocol document describes additional commands, but unverified model-specific writes are not promoted to tested support.
- Summary: Gootu GT-H2436M14P5 is a 24V 3.6kW hybrid inverter observed behind an EyeBond Wi-Fi collector. A support archive and SmartValue proxy capture establish the EyeBond G-ASCII telemetry family, the exact 230V/15A/24V/50Hz plus SVFW 4.007 offline fingerprint, and acknowledged CPR00/CPR03 charging-priority writes. The exact runtime profile therefore reuses the established telemetry decoder and exposes only the evidence-backed charging-priority control.
- Variants:
  - `eybond_g_ascii_gootu_fingerprint` — EyeBond G-ASCII 230V/15A/24V/50Hz, software 4.007
    - Descriptors: gootu_gt_h2436m14p5
    - Known firmware: collector AT R001, inverter F 230V 15A 24V 50Hz, inverter SVFW 4.007
    - `gootu_gt_h2436m14p5` → surface `eybond_g_ascii_gootu_gt_h2436m14p5` (driver eybond_g_ascii, variant gootu_gt_h2436m14p5)
      - Protocol: eybond_g_ascii | Detection: anchors (protocol.protocol_id=EYBOND_G_ASCII; rating.output_voltage=230; rating.output_current=15; rating.battery_voltage=24; rating.frequency=50; firmware.software_version=4.007)
      - Tier: full | Read-only: no | Profile: eybond_g_ascii/models/gootu_gt_h2436m14p5.json | Schema: eybond_g_ascii/base.json
      - Capabilities: 23 (tested 1, untested 22); support tiers: conditional 22, standard 1 | Telemetry: 104 measurements, 3 binary sensors
- Known limitations:
  - Only charging priority is exposed as a model-verified control.
  - CPR01 and CPR02 are document-backed members of the verified CPR command family, but the supplied trace directly captured ACK only for CPR00 and CPR03.
  - Output-priority OPR is intentionally omitted until an ACK and resulting behavior are verified on this exact model.
  - The original Cloud + HA probe timeout was reported on 0.3.0-beta.1; release retesting is still required against the current connection lifecycle.
- Evidence: 4 source(s)

### Kevolt — PD0080G-TPM-EU (`kevolt_pd0080g_tpm_eu`)

- Lifecycle: experimental
- Aliases: Kevolt 8 kW Three-Phase Hybrid
- Validation: hardware confirmed, telemetry partial, controls partial
- Coverage: runtime available, cloud unknown, vendor map documented
  - Coverage notes:
    ...
    - Runtime identification uses four local raw anchors: device type 5, protocol version 0x0104, and low-first rated-power words 0x3880/0x0001 (80000 raw; documented 0.1 W scale = 8000 W).
    - Direct hardware feedback confirms this is the 8 kW model, so the runtime now follows the protocol document's 0.1 W rated-power scale. The raw words remain the stable fingerprint anchors.
    - Hardware feedback confirms the exposed battery values already use the expected negative-on-charge sign; the integration preserves that wire sign.
...
- Known limitations:
  ...
  - Battery output power/current retain the hardware-confirmed wire sign; no additional canonical sign projection is inferred.
  ...
  - The commercial Kevolt identity comes from the public issue report; the local fingerprint identifies the compatible 8 kW Deye-family wire layout and may be shared by OEM siblings.
...
### Sandisolar — SD 11KP48V WIFI (`sandisolar_sd_11kp48v_wifi`)
...
- Coverage: runtime available, cloud unknown, vendor map unknown
  - Coverage notes:
    - Issue #27 support evidence detects the inverter at high confidence through the built-in PI30 MAX runtime.
    - QPI reports PI30, QMN reports the generic raw model VMII-NXPW5KW, and QPIRI reports a 6200 W / 48 V 28-field rating.
- Summary: Yingfa YF6.2K-LEL-1B support is confirmed by the issue #27 support archive. The inverter uses the existing PI30 MAX profile over an EyeBond AT-text collector and provides valid rating, live telemetry, warning, mode, and firmware responses.
- Variants:
  - `pi30_max_qpiri` — PI30 MAX 6200W raw-ASCII runtime over EyeBond AT collector
    - Descriptors: pi30_max_qpiri
    - Known firmware: VERFW:00074.09, VERFW2:00000.00
    - `pi30_max_qpiri` → surface `pi30_max_full` (driver pi30, variant pi30_max)
      - Protocol: pi30 | Detection: anchors (protocol.protocol_id=PI30; shape.qpiri_field_count=)
      - Tier: full | Read-only: no | Profile: pi30_ascii/models/pi30_max.json | Schema: pi30_ascii/models/pi30_max.json
      - Capabilities: 20 (tested 20); support tiers: standard 20 | Telemetry: 77 measurements, 16 binary sensors
