...
class CompiledDetectionCatalog:
    ...
    def resolve(
        ...
        *,
        ...
        """Resolve candidates and a safe surface from normalized evidence."""

        normalized_protocol = str(protocol_key or "").strip()
        if normalized_protocol not in self.protocols:
        ...
        complete = tuple(
            descriptor
            for descriptor in viable
            if _descriptor_has_required_evidence(descriptor, evidence)
        )
        if not complete:
        ...
        priority = max(descriptor.priority for descriptor in complete)
        if any(descriptor.priority > priority for descriptor in viable):
            return self._build_resolution(
                protocol_key=normalized_protocol,
                resolution=RESOLUTION_UNRESOLVED,
                candidates=viable,
        ...
        selected = tuple(
            descriptor
            for descriptor in complete
            if descriptor.priority == priority
        )
        candidate_keys = tuple(sorted(descriptor.key for descriptor in selected))
        path = tuple(
            f"{anchor.key}={evidence.get(anchor.key)!r}"
        ...
        surface_keys = {descriptor.surface_key for descriptor in selected}
