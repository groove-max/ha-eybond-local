...
Спосіб підключення колектора, режим керування та режим оновлення сенсорів можна
змінити пізніше в **Підключення та опитування**.
...
2. Виберіть доступне джерело інформації. Активне навчання може знайти локальні
   елементи, а джерело лише для читання збирає дані для підтримки, не змінюючи
   підключення колектора.
3. Прочитайте попередження про безпеку, якщо вибрано активне навчання.
4. Увійдіть у підтримуваний хмарний акаунт для цього одного сеансу, якщо
...
5. Дочекайтеся перевірки доступної інформації.
6. Перегляньте результат. Застосувати можна лише локально підтверджені елементи.
