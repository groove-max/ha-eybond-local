...
- За замовчуванням залишає адресу хмари виробника для роботи разом із Home
  Assistant, або дозволяє спрямувати колектор лише на Home Assistant — це
  оборотна, явна дія, яка ніколи не виконується непомітно.
...
Тестовий код, який ще не увійшов до релізу, також містить обмежений профіль
**EyeBond Short-ASCII family** для протоколу деяких Anern і Maxinn. Він надає
базові показання, оцінку потужності навантаження та, на пристроях із
відповідними відповідями, дані BMS і номінальні параметри. Жива PV-потужність
і керування відсутні. Це визначення сімейства протоколу, не роздрібної
комерційної моделі. Додаткові сутності вимкнені за замовчуванням. Дивіться
[доступні показання й обмеження](docs/user/RUNTIME_AND_INVERTER.md#eyebond-short-ascii-family).
...
| Застосунок виробника перестав показувати дані | Перевірте режим підключення: **Лише Home Assistant** навмисно відключає хмару. Якщо ви й далі використовуєте **Хмара + Home Assistant**, дивіться [відому проблему хмарної телеметрії](#відома-проблема-хмарної-телеметрії) нижче. |
...
### Відома проблема хмарної телеметрії

Деякі користувачі повідомляють, що в режимі **Хмара + Home Assistant** застосунок
виробника перестає оновлювати дані або показує колектор офлайн, хоча локальні
показання Home Assistant продовжують змінюватися. Хмарні оновлення можуть
відновитися самостійно. Проблема відома й досліджується. Її причину та
універсальне виправлення ще не підтверджено.

Це відрізняється від режиму **Лише Home Assistant**, у якому відключення хмари
навмисне. Ні збільшення інтервалу опитування, ні перезапуск не є підтвердженим
універсальним вирішенням періодичних збоїв.

Якщо це сталося, створіть [архів підтримки](docs/user/SUPPORT_ARCHIVE.md) під час
збою, за можливості до перезапуску чи перемикання режиму. Додайте його до
[issue #13](https://github.com/groove-max/ha-eybond-local/issues/13) або вашого
наявного звернення разом із такими відомостями:

- встановлена версія, а для ручної тестової збірки — також коміт;
- час початку проблеми, ваш часовий пояс і час останніх даних у хмарі;
- чи продовжують змінюватися показання Home Assistant і коли відновиться хмара.

Для повідомлення про цей збій не потрібно видаляти інтеграцію чи скидати колектор.
